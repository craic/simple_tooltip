...
class SimpleTooltipTest < ActiveSupport::TestCase
  ...
  test "validate ok if content has no <script> tag" do
    tooltip = SimpleTooltip.new(:title => 'foo', :markup => 'html', 
  ...
  test "validate with error if format of locale is not two letter" do
    tooltip = SimpleTooltip.new(:title => 'foo', :markup => 'html', :locale => 'locale', 
                                :content => "text")
    assert !tooltip.valid?
  ...
  test "validate OK if format of locale is two letter" do
    tooltip = SimpleTooltip.new(:title => 'foo', :markup => 'html', :locale => 'en', 
                                :content => "text")
    assert tooltip.valid?
  end
  ...
  test "validate fails unless unique title and locale" do
    tooltip0 = SimpleTooltip.new(:title => 'foo', :markup => 'html', :locale => 'en',
                                 :content => "<p>Here to help</p>")
    tooltip0.save!
    
    tooltip1 = SimpleTooltip.new(:title => 'foo', :markup => 'html', :locale => 'en',
                                 :content => "<p>Here to help</p>")
  ...
  test "validate passes duplicate title but unique locale" do
    ...
    tooltip1 = SimpleTooltip.new(:title => 'foo', :markup => 'html', :locale => 'de',
                                 :content => "<p>Here to help</p>")
  ...
  test "content_to_html converts Markdown content to html" do
    ...
    tooltip = SimpleTooltip.new(:title => 'foo', :markup => 'markdown', 
                                :content => markdown)
    assert_equal tooltip.content_to_html, content
  end
  ...
  test 'class method from_title_and_locale return the correct tooltip' do
    ...
    tooltip_out = SimpleTooltip.from_title_and_locale('foo', 'de')
